...
function appendLeg(spec: RobotSpecification, side: "left" | "right", hipY: number) {
  const p = `${side}_`;
  // Six-axis leg topology: yaw/roll/pitch hip, knee, pitch/roll ankle.
  // Carrier sizes and offsets are our layout assumptions, not OP3 dimensions.
  spec.links.push(link(`${p}hip_yaw_carrier`,cyl(.023,.016),{mass:.12,material:"accent_mat",role:"joint_housing"}));
  ...
  spec.joints.push(joint(`${p}ankle_roll`,"revolute",`${p}ankle_pitch_carrier`,`${p}foot`,{axis:[1,0,0],lower:-.35,upper:.35,effort:12,velocity:1.5}));
  spec.joints.push(joint(`${p}ankle_joint`, "revolute", `${p}shin`, `${p}ankle_pitch_carrier`, { origin: pose([0, 0, -SHIN]), axis: [0, 1, 0], lower: -0.8, upper: 0.8 }));
}

export interface HumanoidOptions { name?: string; armDof?: number; gripper?: boolean; legs?: boolean; housing?: boolean; prompt?: string; }
...
export function humanoid(opts: HumanoidOptions = {}): RobotSpecification {
  ...
  if(opts.housing!==false)hollowChassis(spec,"torso");
