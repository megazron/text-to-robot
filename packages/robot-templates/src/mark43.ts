...
export function ironManMark43(opts: ExosuitOptions = {}): RobotSpecification {
  ...
  for (const [s, sign] of SIDES) {
    ...
    part(`${s}_chest_door`, "spine_frame", mesh("chest_plate", `${s}_chest_door.stl`, { w: 0.46, h: 0.30, d: 0.16, t: 0.006, a0: sign > 0 ? 0 : -1.25, a1: sign > 0 ? 1.25 : 0 }),
  ...
  for (const [s, sign] of SIDES) {
    ...
    part(`${s}_thumb`, `${s}_hand`, mesh("armour_plate", `${s}_thumb.stl`, { w: 0.014, h: 0.05, t: 0.012, R: 0.5, corner: 0.004 }), [0.035, fy - sign * 0.058, -0.03], [0, -PI / 2, -sign * 0.8], GOLD, 0.03, { axis: [0, 1, 0], open: -0.9, effort: 1, origin: [0, 0, 0.025] });
  }
  ...
  for (const [s, sign] of SIDES) {
    ...
    part(`${s}_knee_cap`, `${s}_knee_module`, mesh("dome", `${s}_knee_cap.stl`, { radius: 0.065, height: 0.025, thick: 0.005 }), [0.075, ly, 0], [0, PI / 2, 0], RED, 0.30, { axis: [0, 1, 0], open: -0.6, effort: 2 });
    ...
    part(`${s}_shin_stripe`, `${s}_shin_shell`, mesh("armour_plate", `${s}_shin_stripe.stl`, { w: 0.065, h: 0.065, t: 0.003, R: 0.10, corner: 0.008 }), [sr + 0.001, 0, -0.075], [0, 0, 0], GOLD, 0.05);
    part(`${s}_thigh_stripe`, `${s}_thigh_shell`, mesh("armour_plate", `${s}_thigh_stripe.stl`, { w: 0.085, h: 0.21, t: 0.003, R: 0.115, corner: 0.008, taper:0.35 }), [tr + 0.001, 0, -0.15], [0, 0, 0], GOLD, 0.05);
