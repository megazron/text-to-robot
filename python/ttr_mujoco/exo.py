...
def add_wearer(mjcf_xml: str, height: float = 1.75, mass: float = 75.0, undersuit: bool = True,
    ...
    ET.SubElement(tor, "geom", type="capsule", fromto=f"0 0 0.02 0 0 {torsoLen*0.42:.4f}", size="0.105", mass=f"{m('torso')*0.4:.3f}", rgba=suit, contype="0", conaffinity="0")
    ET.SubElement(tor, "geom", type="box", size=f"0.095 {shoulderW*0.5*0.9:.3f} {torsoLen*0.22:.4f}", pos=f"0 0 {torsoLen*0.68:.4f}", mass=f"{m('torso')*0.5:.3f}", rgba=suit, contype="0", conaffinity="0")
    ET.SubElement(tor, "geom", type="capsule", fromto=f"0 {-shoulderW*0.5:.3f} {torsoLen*0.93:.4f} 0 {shoulderW*0.5:.3f} {torsoLen*0.93:.4f}", size="0.045", mass=f"{m('torso')*0.1:.3f}", rgba=suit, contype="0", conaffinity="0")
    ...
    for s, sign in (("left", 1), ("right", -1)):
        ...
        ET.SubElement(sh, "geom", type="capsule", fromto=f"0 0 -0.02 0 0 {-shank+0.03:.4f}", size="0.055", mass=f"{m('shank'):.3f}", rgba=suit, contype="0", conaffinity="0")
        ...
        ET.SubElement(fa, "geom", type="capsule", fromto=f"0 0 -0.02 0 0 {-farm+0.02:.4f}", size="0.037", mass=f"{m('forearm'):.3f}", rgba=suit, contype="0", conaffinity="0")
